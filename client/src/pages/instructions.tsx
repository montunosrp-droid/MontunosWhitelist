...
export default function Instructions() {
  ...
  const handleStart = async () => {
    // El callback ahora trae ?id=DISCORD_ID (y opcionalmente ?resume=1)
    const params = new URLSearchParams(window.location.search);
    const discordId = params.get("id") ?? "";

    try {
      const resp = await fetch("/api/whitelist/start", { method: "POST" });
      const data = await resp.json().catch(() => ({}));

      if (!resp.ok) {
        if (data?.error === "cooldown") {
          const left = data?.hoursLeft ?? 12;
          return setLocation(`/cooldown?left=${left}`);
        }
        if (data?.error === "already_whitelisted") {
          return setLocation("/already-whitelisted");
        }
        return setLocation("/?error=start_failed");
      }

      const formId = data?.formId ?? "1";
      // Mandamos el ID en el query string para prefill del Google Form
      const idQuery = discordId ? `&id=${encodeURIComponent(discordId)}` : "";
      setLocation(`/whitelist-form?f=${encodeURIComponent(formId)}${idQuery}`);
    } catch (e) {
      console.error(e);
        ...
        <Card className="shadow-xl bg-slate-900/85 border border-orange-500/50 rounded-2xl overflow-hidden">
          ...
          <CardContent className="space-y-4 text-sm">
            <ul className="list-disc list-inside space-y-2 text-slate-200 text-left">
              <li>
                Tendrás <strong className="text-orange-300">25 minutos</strong> para completar el formulario.
              </li>
              <li>
